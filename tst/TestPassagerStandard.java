...
class TestPassagerStandard {
  ...
  public void testInteractionMontee() {
    ...
    Transport faux = new FauxVehicule(FauxVehicule.VIDE);
    p.monterDans(faux);

    assert "monteeDemanderAssis" == getLastLog((FauxVehicule)faux) : "assis";

    faux = new FauxVehicule(FauxVehicule.DEBOUT);
    p.monterDans(faux);

    assert "monteeDemanderDebout" == getLastLog((FauxVehicule)faux) : "debout";

    faux = new FauxVehicule(FauxVehicule.PLEIN);
    p.monterDans(faux);

    assert 0 == ((FauxVehicule)faux).logs.size() : "pas de place";
  }
